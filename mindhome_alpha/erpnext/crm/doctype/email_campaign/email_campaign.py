...
class EmailCampaign(Document):
    def validate(self):
        self.set_date()
        # checking if email is set for lead.
        # Not checking for contact as email is a mandatory field for contact.
    ...
    def set_date(self):
        ...
        try:
            ...
            exception = "Please set up the Campaign Schedule in the Campaign {0}"
            frappe.throw(_(exception).format(self.campaign_name))
...
def send_email_to_leads_or_contacts():
    ...
    for camp in email_campaigns:
        ...
        for entry in campaign.get("campaign_schedules"):
            scheduled_date = add_days(
                email_campaign.get("start_date"), entry.get("send_after_days")
            )
...
def send_mail(entry, email_campaign):
    ...
    context = {
        "doc": frappe.get_doc(
            email_campaign.email_campaign_for, email_campaign.recipient
        )
    }
...
def unsubscribe_recipient(unsubscribe, method):
    if unsubscribe.reference_doctype == "Email Campaign":
        frappe.db.set_value(
            "Email Campaign", unsubscribe.reference_name, "status", "Unsubscribed"
        )
...
def set_email_campaign_status():
    email_campaigns = frappe.get_all(
        "Email Campaign", filters={"status": ("!=", "Unsubscribed")}
    )
