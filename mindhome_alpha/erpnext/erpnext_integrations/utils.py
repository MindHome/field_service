    ...
    def innerfn(fn):
        ...
        if (
            ...
            if (
                ...
                and not sig == bytes(frappe.get_request_header(hmac_key).encode())
            ):
...
def get_webhook_address(connector_name, method, exclude_uri=False):
    endpoint = "erpnext.erpnext_integrations.connectors.{0}.{1}".format(
    ...
    server_url = "{uri.scheme}://{uri.netloc}/api/method/{endpoint}".format(
        uri=urlparse(url), endpoint=endpoint
    )
