...
SHIP_ACCOUNT_HEAD = frappe.db.get_single_value(
...
def create_transaction(doc, method):
    ...
    sales_tax = sum(
        [tax.tax_amount for tax in doc.taxes if tax.account_head == TAX_ACCOUNT_HEAD]
    )
...
def get_tax_data(doc):
    ...
    shipping = sum(
        [tax.tax_amount for tax in doc.taxes if tax.account_head == SHIP_ACCOUNT_HEAD]
    )
...
def set_sales_tax(doc, method):
    ...
    if not tax_dict:
        ...
        setattr(
            doc,
            "taxes",
            [tax for tax in doc.taxes if tax.account_head != TAX_ACCOUNT_HEAD],
        )
        return
