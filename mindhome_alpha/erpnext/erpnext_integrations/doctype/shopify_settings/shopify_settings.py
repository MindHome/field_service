...
class ShopifySettings(Document):
    ...
    def validate_access_credentials(self):
        if not (
            ...
            and self.api_key
            and self.shopify_url
        ):
            frappe.msgprint(
                _("Missing value for Password, API Key or Shopify URL"),
    ...
    def register_webhooks(self):
        ...
        for method in webhooks:
            ...
            try:
                ...
                make_shopify_log(
                    status="Warning", exception=error_message, rollback=True
                )
    ...
    def unregister_webhooks(self):
        ...
        for d in self.webhooks:
            url = get_shopify_url(
                "admin/api/2020-04/webhooks/{0}.json".format(d.webhook_id), self
            )
            try:
...
def get_series():
    return {
        "sales_order_series": frappe.get_meta("Sales Order").get_options(
            "naming_series"
        )
        or "SO-Shopify-",
        "sales_invoice_series": frappe.get_meta("Sales Invoice").get_options(
            "naming_series"
        )
        or "SI-Shopify-",
        "delivery_note_series": frappe.get_meta("Delivery Note").get_options(
            "naming_series"
        )
        or "DN-Shopify-",
    }
